						...
						$excerpt = get_the_excerpt();
							...
							if ($title_attribute != '' || $excerpt != '') {
								...
								$remove_link = $photograph_settings['photograph_slider_link'];
									if($remove_link == 0){
										if ($title_attribute != '') {
											echo '<h2 class="slider-title">Hey<a href="'.esc_url(get_permalink()).'" title="'.the_title_attribute('echo=0').'" rel="bookmark">'.get_the_title().'</a></h2><!-- .slider-title -->';
										}
									}else{
										echo '<h2 class="slider-title">Hey'.get_the_title().'</h2><!-- .slider-title -->';
									}
